...
/*
int run_position(double* branching_factors, const char* position) {
  move_t moves[32];
  size_t times_ns[32];
  load_position(&chess_state, position);
  int depth =
      find_best_move_iterative_deepending_test(&chess_state, moves, times_ns);
  for (int i = 1; i < depth; i++) {
    branching_factors[i - 1] = (double)times_ns[i] / (double)times_ns[i - 1];
  }
  return depth;
}
*/
void run_position(const char* filename, const char* position) {
    move_t moves[32];
    size_t times_ns[32];
    load_position(&chess_state, position);
    int depth = find_best_move_iterative_deepening_test(&chess_state, moves, times_ns);
    ...
    // Write the branching factors to the file
    for (int i = 1; i < depth; i++) {
        double bf = (double)times_ns[i] / (double)times_ns[i - 1];
        fprintf(file, "'%s', %d, %.6f\n", position, i, bf);
    }
...
int main(int argc, char const* argv[]) {
  double avg_bf[32];
  int counts[32];
  memset(avg_bf, 0, sizeof(avg_bf));
  memset(counts, 0, sizeof(counts));
  int count = sizeof(positions) / sizeof(const char*);
  for (int _ = 0; _ < 3; _++) {
    for (int i = 0; i < count; i++) {
      run_position("branching-factors.csv", positions[i]);
    }
  }
  for (int i = 0; i < 32; i++) {
    if (counts[i] == 0) break;
    avg_bf[i] /= (double)counts[i];
    printf("depth: %d\taverage branching factor: %f\n", i, avg_bf[i]);
  }
