...
int under_attack(const chess_state_t* chess_state, sq0x88_t square,
  ...
  sq0x88_t king_square = chess_state->enemy_pieces->king_square;
  if (king_increment(king_square, square)) return 1;

  // queen moves
  for (int p = 0; p < chess_state->enemy_pieces->queen_count; p++) {
    sq0x88_t from = chess_state->enemy_pieces->queen_list[p];
    sq0x88_t inc = queen_increment(from, square);
    if (inc == 0) continue;
    sq0x88_t next = backwards_ray_cast(chess_state, square, inc);
    if (next == from) return 1;
    if (piece(chess_state, next) == (KING | chess_state->friendly_colour) &&
        backwards_ray_cast(chess_state, next, inc) == from)
      return 1;
  }

  // rook moves
  for (int p = 0; p < chess_state->enemy_pieces->rook_count; p++) {
    sq0x88_t from = chess_state->enemy_pieces->rook_list[p];
  ...
  if (is_light_square(square)) {
    // light bishop moves
    for (int p = 0; p < chess_state->enemy_pieces->light_bishop_count; p++) {
      sq0x88_t from = chess_state->enemy_pieces->light_bishop_list[p];
    ...
    // dark bishop moves
    for (int p = 0; p < chess_state->enemy_pieces->dark_bishop_count; p++) {
      sq0x88_t from = chess_state->enemy_pieces->dark_bishop_list[p];
      sq0x88_t inc = bishop_increment(from, square);
      if (inc == 0) continue;
      sq0x88_t next = backwards_ray_cast(chess_state, square, inc);
      if (next == from) return 1;
  ...
  // knight moves
  for (int p = 0; p < chess_state->enemy_pieces->knight_count; p++) {
    sq0x88_t from = chess_state->enemy_pieces->knight_list[p];
  ...
  if (!off_the_board(from) &&
      piece(chess_state, from) == (chess_state->enemy_colour | PAWN))
    return 1;
  from = square - inc - (sq0x88_t)1;
  if (!off_the_board(from) &&
      piece(chess_state, from) == (chess_state->enemy_colour | PAWN))
