...
long write_piece(char* buffer, size_t buffer_size, piece_t piece) {
  ...
  switch (piece & PIECE_MASK) {
    ...
    default:     WRITE_ERROR("invalid piece type %02x\n.", piece);
  }
  buffer[0] = piece_char;
  buffer[1] = '\0';
  return 1;
...
long read_piece(const char* buffer, size_t buffer_size, piece_t* piece) {
  *piece = EMPTY;
  ...
  switch (piece_char) {
    ...
    default: READ_ERROR("letter \'%c\' is not a valid piece.\n", piece_char)
  }
  return 1;
}
...
long write_algebraic_notation(char* buffer, size_t buffer_size,
                              chess_state_t* chess_state, move_t move_to_write) {
  // castling
  ...
  if ((moved_piece & PIECE_MASK) != PAWN) {
    out = write_piece(buffer + bytes_written, buffer_size - bytes_written, moved_piece);
    if (out == -1) {buffer[0] = '\0'; return -1;}
    bytes_written += out;
  }
  // disambiguation
  ...
  switch (moved_piece & PIECE_MASK) {
    case PAWN: {
      if (!is_capture(move_to_write)) break; // non capture pawn moves don't need disambiguation
      disambiguate_file = 1; // always disambiguate pawn captures
    } break;
    case KNIGHT: {
      disambiguate_knight(chess_state, from, to , &disambiguate_rank, &disambiguate_file);
    } break;
    case BISHOP: {
    ...
    case KING: break; // no need to disambiguate king moves
    default: {buffer[0] = '\0'; return -1;} // invalid piece (probably already caught in previous step)
  }
  if (disambiguate_file) {
    if (buffer_size - bytes_written < 2) WRITE_ERROR("insufficient space in buffer.\n");
  ...
  // to square
  out = write_square(buffer + bytes_written, buffer_size - bytes_written, get_to(move_to_write));
  ...
  if (is_promotion(move_to_write)) {
    ...
    buffer[bytes_written] = '\0';
    out = write_piece(buffer + bytes_written, buffer_size - bytes_written, get_promotes_to(move_to_write));
    if (out == -1) {buffer[0] = '\0'; return -1;}
    bytes_written += out;
  }
...
long read_algebraic_notation(const char* buffer, size_t buffer_size,
                             const chess_state_t* chess_state, move_t* read_move) {
  if (buffer_size == 0) {
  ...
  *read_move = null_move;
  ...
  if (isupper(piece_char)) {
    switch (piece_char) {
      case 'K': moved_piece = KING;   break;
      case 'Q': moved_piece = QUEEN;  break;
      case 'R': moved_piece = ROOK;   break;
      case 'B': moved_piece = BISHOP; break;
      case 'N': moved_piece = KNIGHT; break;
      default: READ_ERROR("the letter \'%c\' is not a valid piece.\n", piece_char);
    }
    bytes_read++;
  }
  ...
  if (disambiguation_file != -1 && disambiguation_rank != -1 && to_square_file == -1 && to_square_rank == -1) {
  ...
  switch (moved_piece & PIECE_MASK) {
    ...
    case KING: {
      ...
      if (king_increment(from, to) == 0 || !is_legal(chess_state, move(from, to, QUIET_MOVE))) return -1;
    } break;
    default: return -1;
  }
  ...
  if (bytes_read < buffer_size && buffer[bytes_read] == '=') {
    if (moved_piece != PAWN && sq0x88_to_rank07(to) != 0 && sq0x88_to_rank07(to) != 7) {
    ...
    switch (buffer[bytes_read]) {
      case 'Q': flags |= QUEEN_PROMOTE_TO;  break;
      case 'R': flags |= ROOK_PROMOTE_TO;   break;
      case 'B': flags |= BISHOP_PROMOTE_TO; break;
      case 'N': flags |= KNIGHT_PROMOTE_TO; break;
      default: READ_ERROR("the letter \'%c\' is not a valid promotion.\n", buffer[bytes_read]);
...
long write_long_algebraic_notation(char* buffer, size_t buffer_size,
  ...
  if (is_promotion(move)) {
    out = write_piece(buffer + bytes_read, buffer_size - bytes_read, get_promotes_to(move));
    if (out == -1) {buffer[0] = '\0'; return -1;}
    bytes_read += out;
  }
  return bytes_read;
...
long read_long_algebraic_notation(const char* buffer, size_t buffer_size,
  ...
  *read_move = null_move; // default to null_move
  ...
  if (piece(chess_state, from) & PAWN) {
    ...
    if (to - from == chess_state->up_increment * 2) {
        move_flags = DOUBLE_PAWN_PUSH;
    }
    if (sq0x88_to_rank07(to) == 7 || sq0x88_to_rank07(to) == 0) {
      move_flags |= PROMOTION;
      // read promotion
      out = read_piece(buffer + bytes_read, buffer_size - bytes_read, &promotion);
      if (out == -1) return -1;
      bytes_read += out;

      switch (promotion) {
        case QUEEN: move_flags |= QUEEN_PROMOTE_TO; break;
...
long write_movetext_debug(char* buffer, size_t buffer_size,
  ...
  for (int i = 0; i < chess_state->ply_counter; i++) {
    ...
    out = write_long_algebraic_notation(buffer + bytes_written, buffer_size - bytes_written, move);
    if (out == -1) break;
    bytes_written += out;
...
long write_movetext(char* buffer, size_t buffer_size,
  ...
  for (int i = 0; i < chess_state->ply_counter; i++) {
    move_t move = chess_state->ply_stack[i].move;
    out = write_algebraic_notation(buffer + bytes_written, buffer_size - bytes_written, &state, move);
...
long write_pgn(char* buffer, size_t buffer_size,
  ...
  if (fen != NULL) {
    bytes_written += snprintf(buffer + bytes_written, buffer_size - bytes_written, "[FEN \"%s\"]\n", fen);
  }
  if (bytes_written != buffer_size) buffer[bytes_written++] = '\n'; // extra newline before movetext
