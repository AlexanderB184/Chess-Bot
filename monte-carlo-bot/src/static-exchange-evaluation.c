...
void update_xray_attackers(const chess_state_t* chess_state,
  ...
  if (!(bishop_increment(from, to) && attacker & BISHOP) &&
    ...
    return;
  score_centipawn_t value = piece_value(square, attacker);
...
attacker_t next_attacker(attacker_list_t* attackers) {
  score_centipawn_t least_value = attackers->pieces[0].value;
  int least_index = 0;
...
score_centipawn_t static_exchange_evaluation(const chess_state_t* chess_state,
                                             move_t move) {
