...
char * read_response(bot_interface_t* bot_iface, char * buffer, size_t size) {
    ...
    fprintf(stdout, "[BOT] %s", buffer);
    return buffer;
}

// waits for a response that begins with the token "token", ignores all other responses
...
void add_move(char* movetext, size_t space, char* response) {
