...
move_t entry_best_move(entry_t entry) {
  ...
  return move(sq8x8_to_sq0x88(compressed_move & 0x3F), sq8x8_to_sq0x88(((compressed_move) >> 6) & 0x3F),
...
entry_t make_entry(enum tt_entry_type type, move_t best_move, score_cp_t score,
                   int depth, int age) {
  return (entry_t)((entry_t)(type & 0b11) | ((entry_t)(age & 0xFF) << 24) |
                   ((entry_t)(depth & 0xFF) << 16) | ((entry_t)(score & 0xFFFF) << 32) |
                   (((entry_t)(sq0x88_to_sq8x8(best_move.from) & 0x3F) | ((entry_t)(sq0x88_to_sq8x8(best_move.to) & 0x3F) << 6) |
